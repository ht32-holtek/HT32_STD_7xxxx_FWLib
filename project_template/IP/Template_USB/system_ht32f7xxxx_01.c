/**************************************************************************//**
 * @file    library/Device/Holtek/HT32F7xxxx/Source/system_ht32f7xxxx_01.c
 * @brief   CMSIS Cortex-M0+ Device Peripheral Access Layer Source File
 *          for the Holtek HT32F7xxxx Device Series
 * @version $Rev:: 475          $
...
/** @addtogroup HT32F7xxxx_system HT32F7xxxx System
  * @{
  */


#include "ht32f7xxxx_01.h"

/** @addtogroup HT32F7xxxx_System_Private_Defines
  * @{
  */
/*
...
//                <i> PLL feedback clock must be in the range of 24 MHz to 72 MHz
...
//                <i> PLL output clock must be in the range of 4 MHz to 72 MHz
//    </e>
//
...
#define PLL_CLK_SRC_DIV   (0)     /*!< 0: DIV2,     1: DIV4                                                 */
#define PLL_NF2_DIV       (9)     /*!< 1~16: DIV1~DIV16                                                     */
...
/*--------------------- RAMECC Interrupt Configuration ----------------------------------
//
//    <h>RAMECC Interrupt Function and Handler Configuration
//      <o0> Enable RAMECC Interrupt Function and Handler
//           <0=> None
//           <1=> Double/Single Bit Interrupt in FLASH Handler
//           <2=> Double Bit Interrupt in NMI Handler and Single Bit Interrupt in FLASH Handler
//      <o1> Overwrite NMI Handler in ht32Xxxxx_nn_it.c
//           <0=> DISABLE
//           <1=> ENABLE
//           <i> If you want to overwrite the NMI Handler, you have to ENABLE the control option.
//      <o2> Overwrite Flash Handler in ht32Xxxxx_nn_it.c
//           <0=> DISABLE
//           <1=> ENABLE
//           <i> If you want to overwrite the Flash Handler, you have to ENABLE the control option.
//    </h>
*/
#define RAMECC_INTERRUPT                       (0)  /*!< 0: None, 1: FLASH, 2: NMI and FLASH                */
#define RAMECC_NMI_HANDLER_OVER_WRITE_BY_IT    (0)  /*!< 0: DISABLE,  1: ENABLE                             */
#define RAMECC_FLASH_HANDLER_OVER_WRITE_BY_IT  (0)  /*!< 0: DISABLE,  1: ENABLE                             */
...
/** @addtogroup HT32F7xxxx_System_Private_Variables
  * @{
  */
...
/** @addtogroup HT32F7xxxx_System_Private_Functions
  * @{
  */
...
void SystemInit(void)
{
  ...
  ResetBit_BB((u32)(&HT_PWRCU->CR), 4);                         /* set LDO output voltage to level 0 (1.2V) */
  ResetBit_BB((u32)(&HT_PWRCU->CR), 5);

#if (DEINIT_ENABLE == 1)
  /* De-init the setting                                                                                    */
  HT_CKCU->AHBCCR &= ~(3UL << 10);                              /* disable IP who may use PLL as source     */
  ...
  SetBit_BB((u32)(&HT_CKCU->APBCCR1), 6);                       /* Enable Backup domain register clock      */
  if(HT_SYSTEM->RAMCR & 0x00000001)
  {
    if(HT_PWRCU->SR & 0x00000003)
    {
      HT_SYSTEM->RAMCR  = 0x7AC50000;                           /* Disable Write Protection                 */
      HT_SYSTEM->RAMCR  |= 0x00000010;                          /* RAM Erase                                */
      while(HT_SYSTEM->RAMCR & 0x00000010){};                   /* Wait for SRAM Erase                      */
    }
  }
  ResetBit_BB((u32)(&HT_CKCU->APBCCR1), 6);                     /* Disable Backup domain register clock     */
  #if (RAMECC_INTERRUPT != 0)
    HT_SYSTEM->RAMISR = 0x00000003;                             /* Clear RAM ECC interrupt flag             */

    #if (RAMECC_INTERRUPT == 1)
     HT_SYSTEM->RAMICR = 0x00000003;                            /* Config the FLASH interrupt               */
    #endif

    #if (RAMECC_INTERRUPT == 2)
      HT_SYSTEM->RAMICR = 0x00000007;                           /* Config the NMI and FLASH interrupt       */
    #endif

    NVIC_EnableIRQ(FLASH_IRQn);                                 /* Enable FLASH NVIC vector                 */  
  #endif
#endif
...
#if (RAMECC_NMI_HANDLER_OVER_WRITE_BY_IT == 0)
#if (RAMECC_INTERRUPT == 2)
/**
...
#endif
#endif
#if (RAMECC_FLASH_HANDLER_OVER_WRITE_BY_IT == 0)
#if (RAMECC_INTERRUPT != 0)
/**
  * @brief  This function handles Flash interrupt.
  * @retval None
  */
u32 gSingleErrAddress;
void FLASH_IRQHandler(void)
{
  /* Double Error Bit Handling Routine                                                                      */
  /* !!! NOTICE !!! 
     Process Double Error Bit Handling Routine in order, followed by Single Error Bit Handling Routine.
  */
  if(RAMECC_GET_DOUBLE_INT_STATUS())
  {
    while(1)
  ...
  /* Single Error Bit Handling Routine                                                                      */
  if(RAMECC_GET_SINGLE_INT_STATUS())
  {
    gSingleErrAddress = RAMECC_CorrectSingleAddrVal();
  }
}
#endif
#endif
