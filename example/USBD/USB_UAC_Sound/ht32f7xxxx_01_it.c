/*********************************************************************************************************//**
 * @file    USBD/USB_UAC_Sound/ht32f7xxxx_01_it.c
 * @version $Rev:: 338          $
  ...
  if (pTransfer->Buffer)
  {
    switch (state)
    {
      ...
      case I2C_MASTER_TRANSMITTER_MODE:
          ...
          {
            HTCFG_I2CM_ID->TXDR = pTransfer->RegAddr >> 8;
            pTransfer->IsRegAddrSent = FALSE;
          }
          else
          {
            HTCFG_I2CM_ID->TXDR = pTransfer->RegAddr;
      ...
      case I2C_MASTER_TX_EMPTY:
          if (pTransfer->IsRegAddrSent)
          {
            if (pTransfer->Direction == I2CM_DIRECTION_OUT)
            {
              ...
              {
                HTCFG_I2CM_ID->TXDR = pTransfer->Buffer[pTransfer->Counter++];
              }
              else
              {
          ...
          {
            HTCFG_I2CM_ID->TXDR = pTransfer->RegAddr & 0xFF;
            pTransfer->IsRegAddrSent = TRUE;
          }
        break;
      ...
      case I2C_MASTER_RX_NOT_EMPTY:
          ...
          pTransfer->Buffer[pTransfer->Counter++] = HTCFG_I2CM_ID->RXDR;
