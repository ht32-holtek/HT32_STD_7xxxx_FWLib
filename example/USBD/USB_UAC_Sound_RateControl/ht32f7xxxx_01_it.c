...
{
  if (gI2S_RateProcessDownCnt)
    gI2S_RateProcessDownCnt--;
}
  ...
  if (pTransfer->Buffer)
  {
    switch (state)
    {
      ...
      case I2C_MASTER_TRANSMITTER_MODE:
          ...
          {
            I2CM->TXDR = pTransfer->RegAddr >> 8;
            pTransfer->IsRegAddrSent = FALSE;
          }
          else
          {
            I2CM->TXDR = pTransfer->RegAddr;
      ...
      case I2C_MASTER_TX_EMPTY:
          if (pTransfer->IsRegAddrSent)
          {
            if (pTransfer->Direction == I2CM_DIRECTION_OUT)
            {
              ...
              {
                I2CM->TXDR = pTransfer->Buffer[pTransfer->Counter++];
              }
              else
              {
          ...
          {
            I2CM->TXDR = pTransfer->RegAddr & 0xFF;
            pTransfer->IsRegAddrSent = TRUE;
          }
        break;
      ...
      case I2C_MASTER_RX_NOT_EMPTY:
          ...
          pTransfer->Buffer[pTransfer->Counter++] = I2CM->RXDR;
...
void PDMA_CH2_5_IRQHandler(void)
{
  ...
  I2S_BufferRead((u32*)&TxBuf[AltBuf][0]);
